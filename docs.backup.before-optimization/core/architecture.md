...
{
    // Funzionalità comuni per tutti i modelli
}
```
...
```
XotBaseModel → BaseModel (modulo specifico) → Modello concreto
XotBaseResource → Resource (modulo specifico) → Resource concreta
XotBaseRelationManager → RelationManager (modulo specifico) → RelationManager concreto
XotBaseServiceProvider → ServiceProvider (modulo specifico) → ServiceProvider concreto
```
